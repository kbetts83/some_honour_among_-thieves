...
class Room (pygame.sprite.Sprite):
    ...
    def hall_room_check(ra,rb, rla, rlb ):
        ...
        if overlap_hall_testa:
            ...
            if tall == True:
                ra = Room (rla[0],rla[1],rla[2] - Room.hall_thin ,rla[3], rla[4])
                rb = Room (rla[0],rla[1],rla[2] + Room.hall_thin, rla[3], rla[4])

            if long == True:
                ra = Room (rla[0],rla[1],rla[2], rla[3]-Room.hall_thin , rla[4])
                rb = Room (rla[0],rla[1],rla[2], rla[3]+Room.hall_thin , rla[4])
...
print (room_list, Room.hall_list)
